...
export default class extends Vue {
	...
	public goLogin() {
		if (this.$store.state.isLogin) {
			uni.navigateTo({ url: '/pages/step/step' });
		} else {
	...
	private async getHomeConfig() {
		...
		if (result.candidateHomePics != null && result.candidateHomePics.length > 0)
			this.bannerImg = result.candidateHomePics[NumberHelper.getRandomNumInt(0, result.candidateHomePics.length - 1)];
	}
